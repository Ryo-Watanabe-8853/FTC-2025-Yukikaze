        ...
        for (AprilTagDetection detection : currentDetections) {
            if (detection.id == 13) { // タグIDが13の場合
                ...
                if ((currentAngle - targetAngle) > 2) {
                    turnRight();
                    telemetry.addData("robot is","turning right");
                    telemetry.update();
                } else if (currentAngle - targetAngle < -2) {
                    turnLeft();
                    telemetry.addData("robot is","turning left");
                    telemetry.update();
                } else if ((currentDistance - targetDistance) > 10) {
                    moveForward();
                    telemetry.addData("robot is","moving forward");
                    telemetry.update();
                } else if ((currentDistance - targetDistance) < -10) {
                    moveBack();
                    telemetry.addData("robot is","moving backward");
                    telemetry.update();
                } else {
                    stopMotor();
                }
